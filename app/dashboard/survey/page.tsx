...
import { EarthingReadingsTable, ReadingsData } from "@/components/survey/EarthingReadingsTable"
import { ChecklistSection } from "@/components/survey/ChecklistSection"
import { OverallStatusSection, OverallStatus } from "@/components/survey/OverallStatusSection"
import { DocScanBanner } from "@/components/survey/DocScanBanner"
import { SitePhotoCapture, SitePhoto } from "@/components/survey/SitePhotoCapture"
import { ManagerSignature, ManagerSignatureData } from "@/components/survey/ManagerSignature"
...
useEffect(() => {
  ...
  function handleOcrFields(fields: Record<string, string>) {
    ...
    if (fields.overall_status) setOverallStatus(fields.overall_status as OverallStatus)
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!branchValues.bic) { setSubmitError("Branch code is required"); return }
    if (!overallStatus) { setSubmitError("Please set the overall earthing status"); return }
    ...
    <form onSubmit={handleSubmit}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Earthing inspection report · PSB Pan-India</p>
        </div>
        <div className="flex items-center gap-2 bg-white border border-gray-100 rounded-xl px-4 py-2.5 self-start sm:self-auto">
          ...
          <div className="flex flex-col">
            <span className="text-[10px] text-gray-400 uppercase tracking-wide">Survey ID</span>
            <span className="text-xs font-mono font-semibold text-gray-700">{surveyId}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.2fr] gap-5 items-start">
        ...
        <div className="flex flex-col gap-5">
          ...
          <div>
            {submitError && (
              <p className="text-sm text-[#E41E23] mb-3 text-center">{submitError}</p>
            )}
            <Button
              type="submit"
              disabled={submitting}
            ...
            <p className="text-xs text-gray-400 text-center mt-2">
              Saved records appear in the Records tab · Admin can edit submitted records
            </p>
          </div>
        </div>
